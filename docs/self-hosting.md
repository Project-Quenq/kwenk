...
Kwenk runs as one Node process with one SQLite database and one upload directory.
...
```env
KWENK_BASE_URL=https://kwenk.com
KWENK_DATABASE_PATH=./data/kwenk.sqlite
KWENK_UPLOAD_DIR=./data/uploads
KWENK_ADMIN_USER_ID=1
KWENK_MEDIA_CONCURRENCY=1
PORT=3000
HOST=127.0.0.1
```

If these variables are unset, the app defaults to `http://localhost:3000`, `./data/kwenk.sqlite`, `./data/uploads`, admin user id `1`, media concurrency `1`, port `3000`, and host `0.0.0.0`. The account that signs up with `KWENK_ADMIN_USER_ID` receives the `admin` role, and all users are kept as accepted friends with that protected admin account.
...
`KWENK_UPLOAD_DIR` controls where uploads are written. Public user media is served only from the profile-image, post-image, and theme-song buckets at `/media/pfp/*`, `/media/post-images/*`, and `/media/theme-songs/*`. Keep the upload directory on persistent storage and include it in backups.

Uploaded user images are normalized locally before storage. `KWENK_MEDIA_CONCURRENCY` controls sharp/libvips worker concurrency and defaults to `1`, which keeps memory and CPU predictable on small VPS instances.
...
```bash
sqlite3 data/kwenk.sqlite ".backup 'backup/kwenk.sqlite'"
```
...
- Keep `KWENK_BASE_URL` accurate so secure cookies work behind TLS.
